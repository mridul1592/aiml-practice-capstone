...
def audio_query_command(args) -> int:
    """Execute RAG query from audio file."""
    audio_file = args.audio_file
    if not audio_file:
        logger.error("Audio file path is required")
        return 1
    logger.info(f"Processing audio query: {audio_file}")
    try:
        from audio.audio_rag_handler import AudioRAGHandler
        handler = AudioRAGHandler(audio_model_size=args.model_size)
        ...
        print("\n" + "=" * 60)
        print("AUDIO RAG QUERY RESULT")
        print("=" * 60)
        print(f"Audio File: {result['audio_file']}")
        print(f"Audio Duration: {result['audio_duration_seconds']:.2f}s")
        print(f"\nDetected Language: {result['detected_audio_language']}")
        print(f"Audio Confidence: {result['audio_confidence']:.4f}")
        print(f"\nTranscription:")
        print(f"  {result['transcription']}")
        print(f"\nResponse Language: {result['response_language']}")
        print(f"\nResponse:")
        print(result["response"])
        print(f"\nRAG Confidence: {result['rag_confidence']}")
        print(f"Retrieved chunks: {result['num_context_chunks']}")
        print(f"\nSources:")
        for src in result["sources"]:
            print(f"  - {src}")
        ...
        logger.error(f"Error running audio query: {str(e)}")
        print(f"\nError: {str(e)}")
        return 1
...
Examples:
  ...
  python main.py query --query "Paddy irrigation schedule" --crop paddy --region punjab --k 10 --threshold 0.3
    ...
    # Audio query command
    audio_query_parser = subparsers.add_parser("audio-query", help="Run a RAG query from audio file")
    audio_query_parser.add_argument(
        "--audio-file",
        required=True,
        help="Path to audio file (mp3, wav, m4a, flac, ogg, opus, aac)",
    )
    audio_query_parser.add_argument(
        "--model-size",
    ...
    audio_query_parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Respond in audio's detected language (default: True)",
    )
    audio_query_parser.set_defaults(func=audio_query_command)
